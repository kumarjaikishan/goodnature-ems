...
export default function PayrollEdit() {
  ...
  useEffect(() => {
    ...
    setpreviousAdvance((selectedEmployeedetail.advance || 0) + (options?.adjustedAdvance || 0));
  }, [selectedEmployeedetail, options?.adjustedAdvance]);

  useEffect(() => {
    if (!data) return;
    // console.log(data)

    const autoBonusNames = ["Overtime", "Net Overtime"];
