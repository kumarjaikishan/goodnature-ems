...
const EmployeeAdvancePage = () => {
    ...
    const filteredEmployees = rows?.filter((row) => {
        // Only show if an employee is selected
    ...
    const handleOpen = (row = null) => {
        if (row) {
            ...
            // When adding new, use the selectedEmployee context
            setForm({
    ...
    const columns = [
        ...
        { name: "Balance", selector: (r) => r.balance ?? r.remainingBalance, width: "100px" },
                        ...
                        <form onSubmit={handleSubmit}>
                            <span className="modalcontent ">
                                <div className='flex flex-col gap-3 w-full'>
                                    {/* Pre-filled employee name, no dropdown needed when adding as per user request */}
                                    <div className="p-2 bg-gray-100 rounded border text-sm font-medium">
                                        Employee: {selectedEmployee?.userid?.name} ({selectedEmployee?.empId})
                                    </div>
