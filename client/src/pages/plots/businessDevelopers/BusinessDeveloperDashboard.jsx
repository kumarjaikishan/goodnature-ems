...
const BusinessDeveloperDashboard = () => {
  ...
  const fetchDashboardStats = async () => {
    ...
    try {
      ...
      toast.error(err.message || 'Failed to load business developer dashboard');
    } finally {
      setLoading(false);
    }
  };
  ...
  const developer = dashboardData?.sponsor || {};
  ...
  return (
    <div className="p-4 md:p-6 bg-slate-50 min-h-screen flex flex-col gap-5 max-w-7xl mx-auto">
      {/* ── 1. Clean Official Header & Business Developer Profile ────────────────── */}
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          {developer.profileImage ? (
            <img
              src={cloudinaryUrl(developer.profileImage)}
              alt={developer.name}
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-xl font-bold text-slate-800">{developer.name}</h1>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500 mt-1 font-medium">
              <span className="flex items-center gap-1 font-mono">
                ...
                ID: <strong className="text-slate-700">{developer.sponsorCode || 'N/A'}</strong>
              </span>
              {developer.mobile && (
                <span className="flex items-center gap-1 font-mono">
              ...
              {developer.parentSponsor && (
                <span className="flex items-center gap-1">
                  <Users size={13} className="text-slate-400" />
                  Partner: <strong className="text-slate-700">{developer.parentSponsor.name}</strong>
                </span>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        {/* Team Hierarchy / Subordinates (1 Col) */}
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-mono">{sub.sponsorCode || sub.mobile || 'Developer'}</p>
                      </div>
                    </div>
