...
import { Building2, Plus, FileText, ShieldCheck } from 'lucide-react';
...
import KisanSummaryMetrics from './kisanLand/KisanSummaryMetrics';
import KisanAgreementsTable from './kisanLand/KisanAgreementsTable';
import RegistryDeedsTable from './kisanLand/RegistryDeedsTable';
import CreateAgreementModal from './kisanLand/CreateAgreementModal';
import EditAgreementModal from './kisanLand/EditAgreementModal';
import CreateDeedModal from './kisanLand/CreateDeedModal';
import EditDeedModal from './kisanLand/EditDeedModal';
import RecordPaymentModal from './kisanLand/RecordPaymentModal';
import KisanLedgersDrawer from './kisanLand/KisanLedgersDrawer';

const PlotKisanLandPage = () => {
  ...
  // Top Main Navigation Tab: 'agreements' | 'deeds'
  ...
  const fetchAgreements = async () => {
    ...
    try {
      ...
      toast.error('Failed to load Kisan Land Agreements');
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadAgreementDetails = async (id, targetTab = 'overview') => {
    ...
    try {
      ...
      toast.error('Failed to load agreement full ledger');
    }
  };
  ...
  const handleSaveAgreementEdit = async (e) => {
    ...
    try {
      ...
      toast.success('Kisan Land Agreement updated successfully!');
  ...
  const handleDeleteAgreement = async (agr) => {
    const proceed = await confirmDialog({
      title: `Delete Agreement #${agr.agreementNumber}?`,
    ...
    try {
      ...
      toast.success(`Agreement #${agr.agreementNumber} deleted successfully`);
  ...
  if (loading && agreements.length === 0) {
    return (
      <PageLoader
        title="Loading Land Agreements & Deeds..."
  ...
  return (
    <div className="p-4 md:p-6 bg-slate-50 min-h-screen space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-3 border-b border-slate-200">
        ...
        >
          <FileText size={17} />
          Kisan Land Agreements ({agreements.length})
        </button>
...
export default PlotKisanLandPage;
