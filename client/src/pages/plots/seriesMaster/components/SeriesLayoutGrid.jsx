                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-3">
                  {seriesPlots.map((p) => {
                    ...
                    const srcLabel = firstSrc
                      ...
                      : null;
                    const heads = Array.isArray(p.premiumHeads) ? p.premiumHeads : [];
                    const hasPremium = heads.length > 0 || isCorner;
                    const totalExtra = heads.reduce((sum, h) => sum + (Number(h.extraPercent) || 0), isCorner && heads.length === 0 ? (rateConfig?.cornerExtraPercent || 20) : 0);
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-1">
                          ...
                          {hasPremium && (
                            <span
                              className="text-[0.52rem] bg-teal-800 text-white font-black px-1.5 py-0.5 rounded leading-none truncate max-w-[65px]"
                              title={heads.map(h => `${h.name} (+${h.extraPercent}%)`).join(', ') || `Corner (+${totalExtra}%)`}
                            >
                              {heads.length === 1 ? heads[0].name.toUpperCase() : `+${totalExtra}% PLC`}
                            </span>
                          )}
                        </div>
