...
const EditAgreementModal = ({
  ...
  const handleParcelChange = (idx, field, val) => {
    const updated = [...(editAgrForm.landParcels || [])];
    ...
    setEditAgrForm({ ...editAgrForm, landParcels: updated });
  };
  ...
  const addParcelRow = () => {
    const defaultMauja = editAgrForm.landParcels?.[0]?.mauja || editAgrForm.mauja || '';
    const defaultThana = editAgrForm.landParcels?.[0]?.thanaNumber || editAgrForm.thanaNumber || '';
    const defaultRate = editAgrForm.landParcels?.[0]?.ratePerDismil || editAgrForm.ratePerDismil || '';

    setEditAgrForm({
      ...editAgrForm,
      landParcels: [
        ...(editAgrForm.landParcels || []),
        {
          mauja: defaultMauja,
  ...
  const removeParcelRow = (idx) => {
    if ((editAgrForm.landParcels || []).length <= 1) {
      toast.warning('Agreement must have at least one land parcel.');
      return;
    }
    setEditAgrForm({
      ...editAgrForm,
      landParcels: editAgrForm.landParcels.filter((_, i) => i !== idx),
    });
  };

  // ─── FARMERS HANDLERS ──────────────────────────────────────────
  const handleFarmerChange = (idx, field, val) => {
    const updated = [...(editAgrForm.farmers || [])];
    updated[idx] = { ...updated[idx], [field]: val };
    setEditAgrForm({ ...editAgrForm, farmers: updated });
  };
  ...
  const removeFarmerRow = (idx) => {
    if ((editAgrForm.farmers || []).length <= 1) return;
    setEditAgrForm({
      ...editAgrForm,
      farmers: editAgrForm.farmers.filter((_, i) => i !== idx),
    });
  };

  // ─── PURCHASERS / BUYERS HANDLERS ────────────────────────────
  const handlePurchaserChange = (idx, field, val) => {
    const updated = [...(editAgrForm.purchasers || [])];
    updated[idx] = { ...updated[idx], [field]: val };
    setEditAgrForm({ ...editAgrForm, purchasers: updated });
  };
  ...
  const removePurchaserRow = (idx) => {
    if ((editAgrForm.purchasers || []).length <= 1) return;
    setEditAgrForm({
      ...editAgrForm,
      purchasers: editAgrForm.purchasers.filter((_, i) => i !== idx),
    });
  };

  // ─── ATTACHMENTS HANDLERS ──────────────────────────────────────
  const handleAttachmentChange = (idx, field, val) => {
    const updated = [...(editAgrForm.attachments || [])];
    updated[idx] = { ...updated[idx], [field]: val };
    setEditAgrForm({ ...editAgrForm, attachments: updated });
  };
  ...
  const removeAttachmentRow = (idx) => {
    setEditAgrForm({
      ...editAgrForm,
      attachments: (editAgrForm.attachments || []).filter((_, i) => i !== idx),
    });
  };
  ...
  const parcels = editAgrForm.landParcels || [];
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    if (parcels.length === 0) {
      toast.error('Please keep at least one land parcel.');
      return;
    }
    handleSaveAgreementEdit(e);
  };
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs bg-teal-50/60 p-3 rounded-2xl border border-teal-200/80">
              ...
              <div className="bg-white p-2.5 rounded-xl border border-teal-100 shadow-2xs">
                <div className="text-[10px] uppercase font-bold text-slate-400">Total Area (Sq.Ft.)</div>
                <div className="text-sm font-black text-slate-800 mt-0.5">
                  {totalSqFt.toLocaleString('en-IN')} <span className="text-[10px] font-normal text-slate-400">Sq.Ft.</span>
                </div>
              </div>
            ...
            {/* Parcel Rows Card List */}
            <div className="space-y-3">
              {parcels.map((parcel, idx) => (
                <div
                  key={idx}
                  className="p-4 bg-slate-50/80 border border-slate-200/90 rounded-2xl space-y-3 relative hover:border-teal-300 transition"
                >
                  <div className="flex items-center justify-between pb-1 border-b border-slate-200/60">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => setActiveChaudhiIdx(activeChaudhiIdx === idx ? null : idx)}
                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-bold transition cursor-pointer border ${
                          activeChaudhiIdx === idx || Object.values(parcel.chaudhi || {}).some(Boolean)
                            ? 'bg-amber-50 text-amber-800 border-amber-300'
                            : 'bg-white text-slate-600 border-slate-200'
                        }`}
                      >
                        <Compass size={13} />
                        <span>Chaudhi (चौहद्दी)</span>
                        {activeChaudhiIdx === idx ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
                      </button>
                      ...
                      {parcels.length > 1 && (
                        <button
                          ...
                          className="text-rose-500 hover:bg-rose-50 p-1.5 rounded-lg text-xs font-bold transition cursor-pointer"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 bg-white p-2.5 rounded-xl border border-slate-200">
                    <div>
                      ...
                      <input
                        ...
                        value={parcel.araziDismil !== undefined ? parcel.araziDismil : ''}
                      ...
                      <span className="text-[10px] text-teal-700 font-semibold mt-0.5 block">
                        ≈ {Math.round((Number(parcel.araziDismil) || 0) * 435.6)} Sq.Ft.
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={parcel.ratePerDismil !== undefined ? parcel.ratePerDismil : ''}
                        onChange={(e) => handleParcelChange(idx, 'ratePerDismil', e.target.value)}
                      />
                      <span className="text-[10px] text-slate-400 mt-0.5 block">
                        Rate/SqFt: ₹{parcel.ratePerSqFt || 0}
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={parcel.totalAmount !== undefined ? parcel.totalAmount : ''}
                  ...
                  {/* Collapsible Chaudhi Boundaries */}
                  {(activeChaudhiIdx === idx || Object.values(parcel.chaudhi || {}).some(Boolean)) && (
                    <div className="p-3 bg-amber-50/70 border border-amber-200/80 rounded-xl space-y-2">
                      <div className="flex items-center gap-1.5 text-[11px] font-bold text-amber-900">
                        ...
                        <span>Chaudhi Boundaries (Directional Landmarks / Road / Adjoining Plot)</span>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        <div>
                          ...
                          <input
                            type="text"
                            placeholder="e.g. Road 20ft / Plot 581"
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            placeholder="e.g. Canal / Plot 583"
                            className="h-7 w-full bg-white border border-amber-200 rounded-md px-2 text-[11px] outline-none"
          ...
          {/* ── SECTION 2: FARMERS / KISAN OWNERS ── */}
          <div className="space-y-3">
            <div className="flex items-center justify-between border-b border-slate-200/80 pb-2">
              <div className="flex items-center gap-2">
                <Users className="text-teal-700" size={18} />
                <h4 className="text-xs md:text-sm font-bold text-slate-800 uppercase tracking-wider">
                  Kisan / Land Owner(s) Details ({(editAgrForm.farmers || []).length})
                </h4>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-2.5">
              {(editAgrForm.farmers || []).map((farmer, idx) => (
                <div key={idx} className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl space-y-2.5 relative">
                  {(editAgrForm.farmers || []).length > 1 && (
                    <button
                      type="button"
                      onClick={() => removeFarmerRow(idx)}
                      className="absolute right-3 top-3 text-rose-500 hover:bg-rose-50 p-1 rounded-lg text-xs font-bold cursor-pointer"
                      title="Remove farmer"
                    >
                      <Trash2 size={15} />
                    </button>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-2.5">
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 mb-0.5">Farmer Full Name *</label>
                      <input
                        type="text"
                        required
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="e.g. Late Mohan Prasad"
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-600 mb-0.5">Share % in Land</label>
                      <input
                        type="number"
                        placeholder="100"
                        className="h-8 w-full bg-white border border-slate-300 rounded-lg px-2 text-xs font-bold text-teal-800 outline-none"
                        value={farmer.sharePercent !== undefined ? farmer.sharePercent : 100}
                        onChange={(e) => handleFarmerChange(idx, 'sharePercent', e.target.value)}
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="h-8 w-full bg-white border border-slate-300 rounded-lg px-2 text-xs font-medium outline-none uppercase"
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="e.g. Vill- Rampur, PO- Danapur"
                        className="h-8 w-full bg-white border border-slate-300 rounded-lg px-2 text-xs font-medium outline-none"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between border-b border-slate-200/80 pb-2">
              <div className="flex items-center gap-2">
                <Users className="text-teal-700" size={18} />
                <h4 className="text-xs md:text-sm font-bold text-slate-800 uppercase tracking-wider">
                  Purchaser / Buyer(s) Details ({(editAgrForm.purchasers || []).length})
                </h4>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-2.5">
              {(editAgrForm.purchasers || []).map((purchaser, idx) => (
                <div key={idx} className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl space-y-2.5 relative">
                  {(editAgrForm.purchasers || []).length > 1 && (
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    <div>
                      ...
                      <input
                        type="text"
                        required
                        placeholder="e.g. Good Nature Pvt Ltd / John Doe"
                        className="h-8 w-full bg-white border border-slate-300 rounded-lg px-2 text-xs font-bold text-slate-800 outline-none"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between border-b border-slate-200/80 pb-2">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs md:text-sm font-bold text-slate-800 uppercase tracking-wider">
                  Land Documents & Attachments ({(editAgrForm.attachments || []).length})
                </h4>
              </div>
              <button
                type="button"
            ...
            {(editAgrForm.attachments || []).length === 0 ? (
              <div className="p-4 bg-slate-50 border border-dashed border-slate-200 rounded-2xl text-center text-xs text-slate-400">
                No documents attached yet. Click "Attach File / Document" to add scans, Khatiyan copies, maps, or affidavits.
              </div>
            ) : (
              <div className="space-y-2.5">
                {(editAgrForm.attachments || []).map((att, idx) => (
                  <div
                    key={idx}
                  ...
                  >
                    <div>
                      ...
                      <input
                        type="text"
                        required
                        placeholder="e.g. Agreement Scan / Khatiyan"
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100">
            <div className="text-xs text-slate-500 font-medium">
              ...
              <strong className="text-teal-800 font-bold">{totalDismil} Dismil</strong> ({parcels.length} plot parcels)
            </div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              >
                {editAgrLoading ? 'Saving Changes...' : 'Save Agreement Changes'}
              </button>
            </div>
          </div>
        </form>
